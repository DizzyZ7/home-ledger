...
class _HouseholdInvitesSectionState extends ConsumerState<HouseholdInvitesSection> {
  ...
  Future<void> _createInvite() async {
    if (_creating) return;
    final expiresInHours = await _chooseLifetime();
    if (expiresInHours == null || !mounted) return;

    setState(() => _creating = true);
    try {
      final invite = await ref.read(householdRepositoryProvider).createInvite(
    ...
    return showDialog<int>(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            _LifetimeOption(hours: 24, label: dialogContext.invitationLifetime24Hours),
            _LifetimeOption(hours: 72, label: dialogContext.invitationLifetime72Hours),
            _LifetimeOption(hours: 168, label: dialogContext.invitationLifetime168Hours),
          ],
        ),
        actions: [
          TextButton(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                Expanded(
                  child: Text(context.invitationsTitle, style: Theme.of(context).textTheme.titleMedium),
                ),
                TextButton.icon(
            ...
            Text(context.invitationCodeSecurityHint, style: Theme.of(context).textTheme.bodySmall),
            const SizedBox(height: 12),
            invites.when(
              loading: () => const LinearProgressIndicator(),
              error: (_, __) => Text(context.l10n.errorGeneric),
              data: (data) => data.isEmpty
                  ? Text(context.noActiveInvitations)
                  : Column(
                      children: [
...
class _LifetimeOption extends StatelessWidget {
  const _LifetimeOption({required this.hours, required this.label});
...
class _InviteCodeDialog extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final localExpiry = invite.invite.expiresAt.toLocal();
    final formattedExpiry = '${localizations.formatMediumDate(localExpiry)} '
    ...
    return AlertDialog(
      ...
      content: Column(
        ...
        children: [
          ...
          const SizedBox(height: 12),
          Text('${context.invitationExpires}: $formattedExpiry'),
...
class _InviteTile extends StatelessWidget {
  const _InviteTile({required this.invite, required this.revoking, required this.onRevoke});
  ...
  Widget build(BuildContext context) {
    final localizations = MaterialLocalizations.of(context);
    final localExpiry = invite.expiresAt.toLocal();
    final formattedExpiry = '${localizations.formatMediumDate(localExpiry)} '
        '${localizations.formatTimeOfDay(TimeOfDay.fromDateTime(localExpiry))}';
    return ListTile(
