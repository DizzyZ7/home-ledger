...
def summary_response(membership: HouseholdMember, active_household_id: str | None) -> HouseholdSummaryResponse:
...
def member_response(membership: HouseholdMember) -> HouseholdMemberResponse:
...
def list_households(session: DbSession, user: CurrentUser) -> list[HouseholdSummaryResponse]:
    query = select(HouseholdMember).options(selectinload(HouseholdMember.household))
    query = query.join(HouseholdMember.household)
    query = query.where(HouseholdMember.user_id == user.id)
    query = query.order_by(Household.created_at.asc())
    memberships = list(session.scalars(query))
    return [summary_response(membership, user.active_household_id) for membership in memberships]
...
def get_current_household(session: DbSession, user: CurrentUser) -> HouseholdDetailResponse:
    ...
    query = select(HouseholdMember).options(selectinload(HouseholdMember.user))
    query = query.where(HouseholdMember.household_id == membership.household_id)
    query = query.order_by(HouseholdMember.created_at.asc())
    members = list(session.scalars(query))
    return HouseholdDetailResponse(
        **summary_response(membership, user.active_household_id).model_dump(),
        members=[member_response(member) for member in members],
    )
...
def rename_current_household(
    ...
    session.commit()
    session.refresh(household)
    return summary_response(owner_membership, user.active_household_id)
...
def remove_household_member(member_user_id: str, session: DbSession, user: CurrentUser) -> None:
    ...
    fallback_query = select(HouseholdMember)
    fallback_query = fallback_query.where(
        HouseholdMember.user_id == member_user_id,
        HouseholdMember.household_id != owner_membership.household_id,
    )
    fallback_query = fallback_query.join(HouseholdMember.household)
    fallback_query = fallback_query.order_by(Household.created_at.asc())
    fallback = session.scalar(fallback_query)
