...
def create_household_invite_code() -> str:
    """Create a shareable code with approximately 100 bits of entropy."""
...
def household_invite_code_hash(code: str) -> str | None:
    ...
    payload = f"homeledger:household-invite:v1:{normalized}".encode()
    return hmac.new(
        settings.invite_code_secret.encode(),
        payload,
        hashlib.sha256,
    ).hexdigest()
